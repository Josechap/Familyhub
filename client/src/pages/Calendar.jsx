...
import { ChevronLeft, ChevronRight, Loader2, X, Clock, Calendar as CalendarIcon, User, Utensils } from 'lucide-react';
    ...
    return (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={onClose}>
            ...
            >
                ...
                <button
                    onClick={onClose}
                    className="w-full mt-6 py-3 bg-primary text-white rounded-xl font-medium hover:bg-primary/80 transition-colors"
                >
                    Done
                </button>
            </div>
            ...
            {!multiDayView ? (
                    ...
                    <div className="card bg-success/10 border border-success/20 animate-slide-up">
                        <div className="flex items-center gap-4">
                            ...
                            <div className="flex-1">
                                <p className="text-xs text-success font-medium uppercase tracking-wide">Dinner</p>
                                <p className="font-semibold text-lg">{dinner.recipe}</p>
                            </div>
                            <span className="text-3xl">{dinner.emoji || '🍽️'}</span>
                        </div>
                    </div>
                )}
                ...
                {dayEvents.length === 0 && !dinner ? (
                    <div className="card text-center py-12">
                        <CalendarIcon size={48} className="mx-auto text-white/20 mb-4" />
                        <p className="text-white/40 text-lg">No events scheduled</p>
                        <p className="text-white/30 text-sm mt-1">Enjoy your free day!</p>
                    </div>
                ) : (
                            ...
                            >
                                <div className="flex items-start gap-4">
                                    ...
                                    <div className="text-center min-w-[60px]">
                                        <p className="text-lg font-bold">{formatTime(event.startHour).split(' ')[0]}</p>
                                        <p className="text-xs text-white/50">{formatTime(event.startHour).split(' ')[1]}</p>
                                    </div>

                                    {/* Event Info */}
                                    <div className="flex-1 min-w-0">
                                        <p className="font-semibold text-lg truncate">{event.title}</p>
                                        <div className="flex items-center gap-3 mt-1">
                                            ...
                                            {members.length > 0 && members[0] !== 'Family' && (
                                                <div className="flex items-center gap-1.5">
                                                    <div className={cn("w-5 h-5 rounded-full flex items-center justify-center text-xs font-bold text-white", colors.bg)}>
                                                        {members[0]?.[0]}
                                                    </div>
