...
const xml_helper_1 = __importDefault(require("./xml-helper"));
class MetadataHelper {
     ...
     */
    static ParseDIDLTrack(didl, host, port = 1400) {
        if (typeof didl === 'undefined')
            return undefined;
        MetadataHelper.debug('Parsing DIDL %o', didl);
        const parsedItem = didl;
        const didlItem = (parsedItem['DIDL-Lite'] && parsedItem['DIDL-Lite'].item) ? parsedItem['DIDL-Lite'].item : parsedItem;
        const track = {
            Album: xml_helper_1.default.DecodeHtml(didlItem['upnp:album']),
            Artist: xml_helper_1.default.DecodeHtml(didlItem['dc:creator']),
            AlbumArtUri: undefined,
            Title: xml_helper_1.default.DecodeHtml(didlItem['dc:title']),
        ...
        if (didlItem['r:streamContent'] && typeof didlItem['r:streamContent'] === 'string' && track.Artist === undefined) {
            ...
            if (streamContent.length === 2) {
                track.Artist = xml_helper_1.default.DecodeHtml(streamContent[0].trim());
                track.Title = xml_helper_1.default.DecodeHtml(streamContent[1].trim());
            }
            else {
                track.Artist = xml_helper_1.default.DecodeHtml(streamContent[0].trim());
                if (didlItem['r:radioShowMd'] && typeof didlItem['r:radioShowMd'] === 'string') {
                    const radioShowMd = didlItem['r:radioShowMd'].split(',');
                    track.Title = xml_helper_1.default.DecodeHtml(radioShowMd[0].trim());
                }
            }
        }
        ...
        if (didlItem.res) {
            ...
            track.TrackUri = xml_helper_1.default.DecodeTrackUri(didlItem.res['#text']);
     ...
     * @memberof MetadataHelper
     */
    static TrackToMetaData(track, includeResource = false, cdudn = 'RINCON_AssociatedZPUDN') {
        var _a, _b, _c;
        if (track === undefined) {
            return '';
        }
        const localCdudn = (_a = track.CdUdn) !== null && _a !== void 0 ? _a : cdudn;
        const protocolInfo = (_b = track.ProtocolInfo) !== null && _b !== void 0 ? _b : 'http-get:*:audio/mpeg:*';
        const itemId = (_c = track.ItemId) !== null && _c !== void 0 ? _c : '-1';
        ...
        if (includeResource)
            metadata += `<res protocolInfo="${protocolInfo}" duration="${track.Duration}">${track.TrackUri}</res>`;
        if (track.AlbumArtUri !== undefined)
            metadata += `<upnp:albumArtURI>${track.AlbumArtUri}</upnp:albumArtURI>`;
        if (track.Title !== undefined)
            metadata += `<dc:title>${track.Title}</dc:title>`;
        if (track.Artist !== undefined)
            metadata += `<dc:creator>${track.Artist}</dc:creator>`;
        if (track.Album !== undefined)
            metadata += `<upnp:album>${track.Album}</upnp:album>`;
        if (track.UpnpClass !== undefined)
            metadata += `<upnp:class>${track.UpnpClass}</upnp:class>`;
    ...
    static GuessMetaDataAndTrackUri(trackUri, spotifyRegion = '2311') {
        ...
        return {
            trackUri: metadata === undefined || metadata.TrackUri === undefined ? trackUri : (_a = xml_helper_1.default.DecodeTrackUri(metadata.TrackUri)) !== null && _a !== void 0 ? _a : '',
            metadata: metadata || '',
        };
    }
    static GuessTrack(trackUri, spotifyRegion = '2311') {
        ...
        if (trackUri.startsWith('file:///jffs/settings/savedqueues.rsq#') || trackUri.startsWith('sonos:playlist:')) {
            ...
            if ((queueId === null || queueId === void 0 ? void 0 : queueId.length) === 1) {
...
MetadataHelper.debug = debug_1.default('sonos:metadata');
