...
const notificationQueue_1 = require("./models/notificationQueue");
/**
 ...
 */
class SonosDeviceNotifications {
    constructor(av, render, events) {
        this.notificationQueue = new notificationQueue_1.NotificationQueue();
    ...
    get debug() {
        if (this.debugger === undefined) {
            this.debugger = debug_1.default(`sonos:notifications:${this.AVTransportService.Host}`);
        }
    ...
    async playQueue(originalState) {
        ...
        if (currentOptions.onlyWhenPlaying === true && !(originalState === models_1.TransportState.Playing || originalState === models_1.TransportState.Transitioning)) {
        ...
        if (currentItem.options.specificTimeout) {
            ...
            const timeout = setTimeout(() => {
                ...
                this.debug('Specific timeout for Notification ("%s") fired already current Timestamp: %o, FireTime: %o', currentName, (new Date()).getTime(), (_a = currentItem.individualTimeout) === null || _a === void 0 ? void 0 : _a.fireTime);
                this.resolvePlayingQueueItem(currentItem, false);
            }, currentItem.options.specificTimeout * 1000);
            currentItem.individualTimeout = new notificationQueue_1.NotificationQueueTimeoutItem(timeout, fireTime);
        }
        ...
        if (currentOptions.metadata === undefined) {
            const guessedMetaData = metadata_helper_1.default.GuessMetaDataAndTrackUri(currentOptions.trackUri);
        ...
        const setAvResult = await this.AVTransportService.SetAVTransportURI({ InstanceID: 0, CurrentURI: currentOptions.trackUri, CurrentURIMetaData: (_a = currentOptions.metadata) !== null && _a !== void 0 ? _a : '' }).catch((reason) => {
        ...
        if (currentOptions.volume !== undefined) {
            ...
            if (currentOptions.delayMs !== undefined) {
                await async_helper_1.default.Delay(currentOptions.delayMs);
            }
        }
        if (currentItem.individualTimeout !== undefined && currentItem.individualTimeout.timeLeft() < 0) {
            this.debug('Specific timeout for Notification ("") fired already current Timestamp: %o, FireTime: %o', currentName, (new Date()).getTime(), (_b = currentItem.individualTimeout) === null || _b === void 0 ? void 0 : _b.fireTime);
            return await this.playNextQueueItem(originalState);
        }
        if (currentItem.generalTimeout !== undefined && currentItem.generalTimeout.timeLeft() < 0) {
            // The Timeout already fired so play next item
            ...
            this.debug('General timeout for Notification ("%s") fired already current Timestamp: %o, FireTime: %o', currentName, (new Date()).getTime(), (_c = currentItem.individualTimeout) === null || _c === void 0 ? void 0 : _c.fireTime);
            return await this.playNextQueueItem(originalState);
        }
        ...
        let remainingTime = currentItem.options.defaultTimeout === undefined
            ? 1800
        ...
        await async_helper_1.default.AsyncEvent(this.Events, models_1.SonosEvents.PlaybackStopped, remainingTime + 5).catch((err) => this.debug(err));
    ...
    resolvePlayingQueueItem(currentItem, resolveValue) {
        if (!currentItem.resolveAfterRevert) {
            ...
            this.notificationQueue.promisesToResolve.push({ promise: currentItem.resolve, value: resolveValue, timeout: currentItem.generalTimeout });
        }
        return true;
    }
    async playNextQueueItem(originalState) {
    ...
    addToNotificationQueue(options, resolve, reject, resolveAfterRevert) {
        const queueItem = {
            options,
            resolve,
            reject,
        ...
        if (options.timeout) {
            ...
            queueItem.generalTimeout = new notificationQueue_1.NotificationQueueTimeoutItem(timeout, fireTime);
        }
    ...
    async startQueue(options) {
        ...
        await this.AVTransportService.GetTransportInfo()
            .then((result) => {
            originalState = result.CurrentTransportState;
        })
            .catch((reason) => {
        ...
        const originalMediaInfoResponse = await this.AVTransportService.GetMediaInfo()
            .catch((reason) => {
            this.debug('Error retrieving initial Media info, but this shouldn\'t stop playing the queue:  %o', reason);
        });
        const originalMediaInfo = (typeof originalMediaInfoResponse === 'object') ? originalMediaInfoResponse : undefined;
        const originalPositionInfoResponse = await this.AVTransportService.GetPositionInfo()
            .catch((reason) => {
            this.debug('Error retrieving initial position info, but this shouldn\'t stop the queue:  %o', reason);
        });
        const originalPositionInfo = (typeof originalPositionInfoResponse === 'object') ? originalPositionInfoResponse : undefined;
        ...
        if (this.notificationQueue.anythingPlayed) {
            ...
            if (originalVolume !== undefined && this.notificationQueue.volumeChanged) {
                ...
                if (options.delayMs !== undefined)
                    await async_helper_1.default.Delay(options.delayMs);
...
exports.default = SonosDeviceNotifications;
//# sourceMappingURL=sonos-notification-two.js.map